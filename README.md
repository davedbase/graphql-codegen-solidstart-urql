...
> **Note:** SolidStart uses different primitives than SolidJS. See [COMPARISON.md](./COMPARISON.md) for differences between this plugin and `typescript-solid-urql`.
...
**Note:** This plugin generates `query*` functions for Query operations and `action*` functions for Mutation operations. Subscriptions are not generated and should be handled directly with `createSubscription` from `@urql/solid-start`.
...
### Subscriptions
Subscriptions are not generated. Use `createSubscription` from `@urql/solid-start` directly with the generated document constants.
